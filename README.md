...
The MATLAB Integration for Jupyter enables you to open a MATLAB Desktop in a web browser from your Jupyter environment. 

`jupyter-matlab-proxy` is a Python® package based on the following packages.
| Package | Description |
|----|----|
| [matlab-desktop-proxy](https://github.com/mathworks/matlab-desktop-proxy) | Provides infrastructure to launch MATLAB and connect to it from a web browser.|
...
* See [Requirements](https://github.com/mathworks/matlab-desktop-proxy#requirements) from `matlab-desktop-proxy`
...
* Open your Jupyter environment by starting jupyter notebook or lab
  ...
  For more information on `MWI_BASE_URL`, refer [Advanced Usage](https://github.com/mathworks/matlab-desktop-proxy/blob/main/Advanced-Usage.md) from `matlab-desktop-proxy`.
...
The package has the same set of limitations as `matlab-desktop-proxy` package. For more information, click [here](https://github.com/mathworks/matlab-desktop-proxy#limitations)
...
* Ensure that `matlab-desktop-proxy-app` and the `jupyter` executables are in the same environment as the python executable.
    ...
    /home/user/my-project/packages/.venv/bin/matlab-desktop-proxy-app

    $ which jupyter
    /home/user/my-project/packages/.venv/bin/jupyter
    ```
    Notice that `matlab-desktop-proxy-app`, `jupyter` and the `python` executable are in the same parent directory, in this case it is: `/home/user/my-project/packages/.venv/bin`
...
* Ensure that all three packages are installed in the same python environment
    ```bash
    # Pipe the output of pip freeze and grep for jupyter, matlab-web-desktop-proxy and jupyter-matlab-proxy.
    ...
    $ pip freeze | grep -E "jupyter|matlab-web-desktop-proxy|jupyter-matlab-proxy"
    ```
...
* If the integration is not showing up as an option to the dropdown box in the Juptyer notebook:
    ...
    config dir: /home/user/anaconda3/etc/jupyter/nbconfig
        ...
        - Validating: OK  $ pip freeze | grep -E "jupyter|matlab-web-desktop-proxy|jupyter-matlab-proxy"
